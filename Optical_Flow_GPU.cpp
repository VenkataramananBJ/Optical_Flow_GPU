...
// VideoCapture cap("M:\\____videoSample____\\tracking\\TownCentreXVID.avi");
 ...
 unsigned long Atime, Btime,Ctime;
 float TakeTime_proc, TakeTime_display;


	//Writes video to a .avi file
	cv::VideoWriter output(
			"data/Optical_Flow_GPU.avi",
			CV_FOURCC('X', 'V', 'I', 'D'),
			30,
 ...
 while (1)
 {
  ...
  Ctime = getTickCount();
  TakeTime_proc = (Btime - Atime) / getTickFrequency();  
  TakeTime_display=(Ctime-Btime) / getTickFrequency();
  printf("Processing Time: %lf sec / %lf fps \n", TakeTime_proc, 1 / TakeTime_proc);
  printf("Display Time: %lf sec / %lf fps \n", TakeTime_display, 1 / TakeTime_display);
